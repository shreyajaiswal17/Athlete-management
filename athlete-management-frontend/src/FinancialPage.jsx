...
function FinancialPlanningPage() {
  ...
  useEffect(() => {
    const fetchFinancialPlanning = async () => {
      try {
        ...
        let analysis = `<h3 class="text-purple-400 text-xl font-semibold">Financial Planning Analysis (Age ${age}, ${sport})</h3>`;
        analysis += `<p class="mt-2 text-lg"><strong class="text-gray-300">Financial Profile:</strong> Income: <span class="text-green-400">$${income.toLocaleString()}</span>, Expenses: <span class="text-red-400">$${expenses.toLocaleString()}</span>, Savings: <span class="text-blue-400">$${savings.toLocaleString()}</span>, Savings Rate: <span class="${savingsRate < 25 ? 'text-yellow-400' : 'text-green-400'}">${savingsRate.toFixed(1)}%</span>, Wins: ${competitionWins}, Injuries: ${injuryCount}.</p>`;
        analysis += `<p class="mt-2 text-lg"><strong class="text-gray-300">Goals:</strong> ${careerGoals.join(', ')}</p>`;

        if (advice === 'Build Wealth') {
          analysis += `<h4 class="text-blue-400 mt-4 text-lg">Build Wealth (Probability: ${Math.round(probabilities[0] * 100)}%)</h4>`;
          analysis += `<ul class="list-disc pl-5 mt-2"><li><strong class="text-gray-300 text-lg">Why:</strong> Your income ($${income.toLocaleString()}) and savings ($${savings.toLocaleString()}) are solid, but a ${savingsRate.toFixed(1)}% savings rate needs growth to secure ${sport} career goals.</li>`;
          analysis += `<li><strong class="text-gray-300 text-lg">Next Steps:</strong> Aim for a <span class="text-green-400">25% savings rate</span> by saving $${savingsTarget.toLocaleString()}/year. Invest in low-risk funds.</li></ul>`;
        } else if (advice === 'Stabilize Finances') {
          analysis += `<h4 class="text-yellow-400 mt-4 text-lg">Stabilize Finances (Probability: ${Math.round(probabilities[1] * 100)}%)</h4>`;
          analysis += `<ul class="list-disc pl-5 mt-2"><li><strong class="text-gray-300 text-lg">Why:</strong> Expenses ($${expenses.toLocaleString()}) are high relative to income ($${income.toLocaleString()}), leaving a ${savingsRate.toFixed(1)}% savings rate.</li>`;
          analysis += `<li><strong class="text-gray-300 text-lg">Next Steps:</strong> Cut expenses by 10-15% (e.g., reduce non-essential costs) to boost savings.</li></ul>`;
        } else if (advice === 'Plan for Retirement') {
          analysis += `<h4 class="text-orange-400 mt-4 text-lg">Plan for Retirement (Probability: ${Math.round(probabilities[2] * 100)}%)</h4>`;
          analysis += `<ul class="list-disc pl-5 mt-2"><li><strong class="text-gray-300 text-lg">Why:</strong> At age ${age} with $${savings.toLocaleString()} saved, you need a long-term plan for post-${sport} career.</li>`;
          analysis += `<li><strong class="text-gray-300 text-lg">Next Steps:</strong> Start a retirement account (e.g., IRA) and contribute $${(income * 0.1).toLocaleString()}/year.</li></ul>`;
        }

        analysis += `<p class="mt-4 text-lg"><strong class="text-gray-300">Projected Savings (10 Years, 5% Growth):</strong> <span class="text-green-400">$${Math.round(savingsIn10Years).toLocaleString()}</span></p>`;
        if (injuryCount > 0) {
          analysis += `<p class="mt-2 text-lg text-yellow-400">Warning: ${injuryCount} injuries may increase costs by $${injuryRiskCost.toLocaleString()}/year—consider insurance.</p>`;
        }
  ...
  const financialChartOptions = {
    ...
    plugins: {
      ...
      title: { display: true, text: 'Financial Overview', color: '#ffffff', font: { size: 20 } },
    },
    scales: {
      y: {
        beginAtZero: true,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#023E8A] via-[#0077B6] text-white p-6">
      ...
      {financialPlanning.advice ? (
        <>
          <p className="text-xl"><strong className="text-gray-300">Financial Advice:</strong> <span className="text-teal-400">{financialPlanning.advice}</span></p>
          <div className="mt-4 text-lg" dangerouslySetInnerHTML={{ __html: financialPlanning.analysis }} />
          <div className="h-96 mt-4">
            <Bar data={financialChartData} options={financialChartOptions} />
          </div>
          <div className="mt-4">
            <h3 className="text-xl font-medium text-blue-400">Probability Breakdown</h3>
        ...
        <p className="text-lg">Generating financial plan...</p>
      )}
      <button
