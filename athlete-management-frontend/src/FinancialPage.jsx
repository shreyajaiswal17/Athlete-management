  ...
  useEffect(() => {
    const fetchFinancialPlanning = async () => {
      try {
        ...
        const expenses = latestPerformance?.expenses || income * 0.75;
        ...
        analysis += `<p className="mt-2"><strong className="text-gray-300">Financial Profile:</strong> Income: <span className="text-green-400">$${income.toLocaleString()}</span>, Expenses: <span className="text-red-400">$${expenses.toLocaleString()}</span>, Savings: <span className="text-blue-400">$${savings.toLocaleString()}</span>, Savings Rate: <span className="${savingsRate < 25 ? 'text-yellow-400' : 'text-green-400'}">${savingsRate.toFixed(1)}%</span>, Wins: ${competitionWins}, Injuries: ${injuryCount}.</p>`;
        analysis += `<p className="mt-2"><strong className="text-gray-300">Goals:</strong> ${careerGoals.join(', ')}</p>`;

        if (advice === 'Build Wealth') {
          analysis += `<h4 className="text-blue-400 mt-4">Build Wealth (Probability: ${Math.round(probabilities[0] * 100)}%)</h4>`;
          analysis += `<ul className="list-disc pl-5 mt-2"><li><strong className="text-gray-300">Why:</strong> Your income ($${income.toLocaleString()}) and savings ($${savings.toLocaleString()}) are solid, but a ${savingsRate.toFixed(1)}% savings rate needs growth to secure ${sport} career goals.</li>`;
          analysis += `<li><strong className="text-gray-300">Next Steps:</strong> Aim for a <span className="text-green-400">25% savings rate</span> by saving $${savingsTarget.toLocaleString()}/year. Invest in low-risk funds.</li></ul>`;
        } else if (advice === 'Stabilize Finances') {
          analysis += `<h4 className="text-yellow-400 mt-4">Stabilize Finances (Probability: ${Math.round(probabilities[1] * 100)}%)</h4>`;
          analysis += `<ul className="list-disc pl-5 mt-2"><li><strong className="text-gray-300">Why:</strong> Expenses ($${expenses.toLocaleString()}) are high relative to income ($${income.toLocaleString()}), leaving a ${savingsRate.toFixed(1)}% savings rate.</li>`;
          analysis += `<li><strong className="text-gray-300">Next Steps:</strong> Cut expenses by 10-15% (e.g., reduce non-essential costs) to boost savings.</li></ul>`;
        } else if (advice === 'Plan for Retirement') {
          analysis += `<h4 className="text-orange-400 mt-4">Plan for Retirement (Probability: ${Math.round(probabilities[2] * 100)}%)</h4>`;
          analysis += `<ul className="list-disc pl-5 mt-2"><li><strong className="text-gray-300">Why:</strong> At age ${age} with $${savings.toLocaleString()} saved, you need a long-term plan for post-${sport} career.</li>`;
          analysis += `<li><strong className="text-gray-300">Next Steps:</strong> Start a retirement account (e.g., IRA) and contribute $${(income * 0.1).toLocaleString()}/year.</li></ul>`;
        }

        analysis += `<p className="mt-4"><strong className="text-gray-300">Projected Savings (10 Years, 5% Growth):</strong> <span className="text-green-400">$${Math.round(savingsIn10Years).toLocaleString()}</span></p>`;
        if (injuryCount > 0) {
          analysis += `<p className="mt-2 text-yellow-400">Warning: ${injuryCount} injuries may increase costs by $${injuryRiskCost.toLocaleString()}/year—consider insurance.</p>`;
        }
