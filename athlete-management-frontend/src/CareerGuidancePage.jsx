...
function CareerGuidancePage() {
  ...
  useEffect(() => {
    const fetchCareerGuidance = async () => {
      try {
        ...
        let analysis = `<h3 class="text-teal-400 text-2xl font-semibold">Career Analysis (Age ${age}, ${sport})</h3><p class="mt-2 text-xl"><strong class="text-gray-300">Profile:</strong> Training <span class="text-yellow-300">${hoursTrained} hours/week</span>, ${sessionsPerWeek} sessions, ${restDays} rest days, ${injuryCount} injuries, ${competitionWins} wins. <em class="text-gray-300">Education:</em> ${education}. <em class="text-gray-300">Goals:</em> ${careerGoals.join(', ')}</p>`;
        
        if (guidance === 'Focus on Training') {
          analysis += `<h4 class="text-blue-400 mt-4 text-xl">Build Elite Potential (Probability: ${Math.round(probabilities[0] * 100)}%)</h4><ul class="list-disc pl-5 mt-2"><li><strong class="text-gray-300 text-xl">Why:</strong> Your ${hoursTrained} hours/week and ${competitionWins} win(s) show promise, but reaching elite ${sport} levels requires a bigger push.</li><li><strong class="text-gray-300 text-xl">Next Steps:</strong> Boost to <span class="text-green-400">${(hoursTrained * 1.2).toFixed(1)} hours/week</span> with ${sport}-specific skills (e.g., ${sport === 'Swimming' ? 'flip turns' : sport === 'Track' ? 'sprint starts' : sport === 'Cricket' ? 'bowling accuracy' : 'core drills'}).</li></ul>`;
        } else if (guidance === 'Competition Ready') {
          analysis += `<h4 class="text-green-400 mt-4 text-xl">Competition Ready (Probability: ${Math.round(probabilities[1] * 100)}%)</h4><ul class="list-disc pl-5 mt-2"><li><strong class="text-gray-300 text-xl">Why:</strong> With ${competitionWins} win(s) and a strong ${hoursTrained} hours/week base, you’re ready to shine in ${sport} events.</li><li><strong class="text-gray-300 text-xl">Next Steps:</strong> Compete quarterly to sharpen skills under pressure.</li></ul>`;
        } else if (guidance === 'Sustain Performance') {
          analysis += `<h4 class="text-orange-400 mt-4 text-xl">Refine and Sustain (Probability: ${Math.round(probabilities[2] * 100)}%)</h4><ul class="list-disc pl-5 mt-2"><li><strong class="text-gray-300 text-xl">Why:</strong> Your ${hoursTrained} hours/week and ${injuryCount} injuries show strength, but balance is key to avoid setbacks.</li><li><strong class="text-gray-300 text-xl">Next Steps:</strong> Keep <span class="text-green-400">${hoursTrained} hours/week</span>, spread into ${Math.min(sessionsPerWeek + 1, 6)} sessions, and add 1-2 rest days.</li></ul>`;
        }
        analysis += `<p class="mt-4 text-xl"><strong class="text-gray-300">Probability Breakdown:</strong> Build Elite Potential (<span class="text-white">${Math.round(probabilities[0] * 100)}%</span>), Competition Ready (<span class="text-green-400">${Math.round(probabilities[1] * 100)}%</span>), Refine and Sustain (<span class="text-orange-400">${Math.round(probabilities[2] * 100)}%</span>)</p>`;
  ...
  if (!athleteData && !error) return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-[#012A4A] via-[#0077B6] to-[#03045E] text-gray-300 text-2xl">
      Loading...
    </div>
  );
  if (!athleteData) return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-[#012A4A] via-[#0077B6] to-[#03045E] text-red-400 text-2xl">
      Athlete not found
    </div>
  );

  return (
    <div className="min-h-screen bg-gradient-to-br from-[#012A4A] via-[#0077B6] to-[#03045E] text-white p-6">
      <h1 className="text-5xl font-bold mb-6">Career Guidance</h1>
      {error && <p className="text-red-400 mb-4 bg-red-900/20 p-2 rounded text-xl">{error}</p>}
      {careerGuidance.guidance ? (
        <>
          <p className="text-2xl"><strong className="text-gray-300">Guidance:</strong> <span className="text-teal-400">{careerGuidance.guidance}</span></p>
          <div className="mt-4 text-xl" dangerouslySetInnerHTML={{ __html: careerGuidance.analysis }} />
          <p className="mt-4 text-2xl"><strong className="text-gray-300">Estimated Time to Goal:</strong> <span className="text-green-400">{careerGuidance.yearsToGoal} years</span></p>
          <div className="h-64 mt-4 w-full">
            <Bar data={careerProbabilityData} options={careerProbabilityOptions} />
          </div>
          <div className="mt-4">
            <h3 className="text-3xl font-medium text-white">Career Tools ({athleteData.sport || 'General'})</h3>
            <ul className="list-disc pl-5 mt-2">
              {careerResources[athleteData.sport]?.[careerGuidance.guidance]?.map((resource, index) => (
                <li key={index} className="text-2xl">
                  <a href={resource.url} target="_blank" rel="noopener noreferrer" className="text-white hover:underline">
                    {resource.name}
                  </a>
                  <span className="text-gray-300"> - {resource.desc}</span>
                </li>
        ...
        <p className="text-xl text-gray-300">Analyzing career guidance...</p>
      )}
      <button
        onClick={() => navigate(`/athlete/${id}`)}
        className="mt-6 bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 transition-colors text-2xl"
      >
